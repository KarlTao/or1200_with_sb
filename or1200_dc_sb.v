...
module or1200_dc_sb (
		     ...
		     //sbdc_tag_o, 
		     sbdc_ci_o,
		     sbdc_data_i, sbdc_ack_i, sbdc_err_i, hit_sb
		     //sbdc_tag_i
		     );
...
`endif
   wire 	 bypass;
   wire 	 disabled;
   assign disabled = !dc_en; //| !dmmu_en | no_sb;
   assign bypass = disabled | (state2 == LOADING) | (state2 == ERR_HANDLING) | (sbdc_eval === 1);
	...
	if (rst == 1'b1 | disabled) begin
	   ...
	   case(state)
		...
		if ((isStore | isLoad) & invalid_addr) begin
		   ...
		   if(fifo_empty) begin 
		      sbdc_eval = 1; end //eval sb and dc hit at the same cycle
		   else begin
		      sbdc_eval = 0; end
		   if(load_hit_sb) begin
		      fifo_wr = 0;
		      next_state = LOAD_HIT_SB;
		   end
		   else  begin //load_miss_sb
 		      fifo_wr = 0;
